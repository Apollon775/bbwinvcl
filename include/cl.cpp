...
char* read_cpuinfo()
{
    ...
    char str[BUFFER];
    char* finalstr = (char*)malloc(BUFFER);
    int i; 
    char buffer;
    int count = 0;
...
hdata_t* hdata_init()
{
    hdata_t *data = (hdata_t*)malloc(sizeof(struct _hostdata));
...
int send_data(int sock_fd, hdata_t *data)
{
    ...
    // -----------Username------------------
    if(send(sock_fd, data->user, strlen(data->user), 0) != strlen(data->user))
    {
        free(buffer);
        return -1;
    }
